...
export default function CheckBookingPage() {
  ...
  const onSubmit = async (data: CheckBookingFormData) => {
    ...
    try {
      // 예약 상세 페이지로 이동 (API는 detail 페이지에서 호출)
      router.push(`/bookings/detail?phone=${data.phoneNumber}&password=${data.password}`);
    } catch (error) {
      toast({
        title: '예약 조회 실패',
    ...
    <div className="container mx-auto py-12 px-4 max-w-md">
      <Card>
        <CardHeader className="text-center">
          ...
          <CardDescription>
            예약 시 입력한 핸드폰 번호와 비밀번호로 예약을 확인하실 수 있습니다.
          </CardDescription>
        </CardHeader>
        <CardContent>
